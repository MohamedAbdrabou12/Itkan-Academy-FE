...
export default function UpdateExam({
  ...
  const form = useForm<FormData>({
    ...
    defaultValues: {
      ...
      duration_minutes: examToEdit?.duration_minutes || 0,
      start_time:
        new Date(examToEdit?.start_time as string)
          .toISOString()
          .split("T")[0] || "",
      end_time:
        new Date(examToEdit?.end_time as string).toISOString().split("T")[0] ||
        "",
  ...
  const { data, isPending: isQuestionsPending } = useGetQuestions();

  const updateExamMutation = useUpdateExam(`${examToEdit?.id}`);
  const publishExamMutation = usePublishExam(`${examToEdit?.id}`);
  const closeExamMutation = useCloseExam(`${examToEdit?.id}`);

  const isPending =
    updateExamMutation.isPending ||
    publishExamMutation.isPending ||
    closeExamMutation.isPending;
            ...
            <form onSubmit={form.handleSubmit(onSubmit)}>
              ...
              <div className="mb-4 flex w-full items-center justify-between">
                ...
                <button
                  ...
                  className="flex cursor-pointer items-center space-x-2 rounded-lg border border-gray-400 px-4 py-2 font-medium transition-colors hover:bg-emerald-700 hover:text-white disabled:disabled:cursor-not-allowed disabled:opacity-50 disabled:hover:bg-transparent disabled:hover:text-black"
                >
                  <span>
